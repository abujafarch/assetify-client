...
const MyAssetFilter = ({ setFilterModalOpen, selectTitle1, selectTitle2, option1, option2, option3, option4, handleFiltering }) => {
    ...
    const handleCollectingFilterValue = (e) => {
        e.preventDefault()
        const form = e.target
        const approvalStatus = form.approvalStatus.value
        const returnability = form.returnability.value
        handleFiltering(approvalStatus, returnability)
        console.log(approvalStatus, returnability);
    }
                ...
                <form onSubmit={handleCollectingFilterValue}>
                    <div className="flex items-center justify-between gap-2 xs:gap-5">
                        <div>
                            ...
                            <select defaultValue="select" className="bg-transparent cursor-pointer border rounded-sm p-1 border-[#192747] text-sm xs:text-base outline-none font-raleway mt-3" name="approvalStatus">
